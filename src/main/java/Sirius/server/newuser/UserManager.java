...
public class UserManager implements Sirius.server.newuser.UserServer
{
    ...
    public User getUser(String userGroupDomain, String userGroup, String userDomain, String user, String password) throws java.rmi.RemoteException, UserException
    {
        ...
        if(u==null)
        {
            throw new UserException("UserException::no User::"+user+","+userDomain+ ","+userGroup+","+userGroupDomain+" registered",true,false,false,false);
        }
        ...
        else // es ist mindestens eine Membership eingetragen
        {
            ...
            for(int i = 0;i<l.size();i++)
            {
                ...
                if(i==l.size()-1) // last element and no break
                {
                    ...
                    throw new UserException("UserException::no UserGroup::"+userGroup+","+userGroupDomain,
    ...
    {
        if(user==null)
        { //throw new java.rmi.RemoteException("UserException::no Usergroup for ::"+userName + " -- "+domain);
            if (logger!=null)logger.debug("no  user "+user);
            return new Vector(0);
        }
    ...
    public java.util.Vector getUserGroupNames(String userName, String domain) throws java.rmi.RemoteException
    {
        ...
        { //throw new java.rmi.RemoteException("UserException::no Usergroup for ::"+userName + " -- "+domain);
            if (logger!=null) logger.debug("no  user "+userName+ "or no domain "+domain);
            return new Vector(0);
        }
        ...
        if(l==null)
        { //throw new java.rmi.RemoteException("UserException::no Usergroup for ::"+userName + " -- "+domain);
            if (logger!=null) logger.debug("no usergroup for user "+userName);
            return new Vector(0);
        }
        ...
        for(int i =0;i<l.size();i++)
        {
            ...
            if(ug!=null && ug.getUgDomain().equalsIgnoreCase(ug.getUserDomain()) ) //fremde Nutzerrechte werden nicht in der Auswahl angeboten
            {
