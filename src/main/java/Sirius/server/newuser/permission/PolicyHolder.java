...
public class PolicyHolder implements Serializable{
    ...
    private HashMap<String, HashMap<Permission, Boolean>> policyHolder = new HashMap<String, HashMap<Permission, Boolean>>();
    ...
    public PolicyHolder(DBConnectionPool conPool) throws ServerExitError{
        ...
        try {
            ResultSet serverPolicies = con.getConnection().createStatement().executeQuery(
                    "SELECT " +
            ...
            if (serverPolicies == null) {
                  log.error("<LS> ERROR :: Serverpolicies konten nicht geladen werden fataler Fehler das Programm beendet sich");
                  throw new ServerExitError("Serverpolicies konten nicht geladen werden fataler Fehler das Programm beendet sich");
            }
