...
public class CatalogueServiceImpl
{
    ...
    public CatalogueServiceImpl(java.util.Hashtable activeLocalServers)
     ...
     public Node[] getChildren(Node node,User user) throws RemoteException
    {
        ...
        try
        {
            ...
            NodeReferenceList c = ( (Sirius.server.middleware.interfaces.domainserver.CatalogueService)activeLocalServers.get( node.getDomain()) ).getChildren(node,user);
            ...
            if (logger!=null) logger.info("<CS> Anzahl Remote links bei getchildren :"+links.length);
            ...
            for(int i =0;i< groupedLinks.length;i++)
            {
                ...
                if (logger!=null) logger.info(lsName+" kriegt anfrage nach knoten");
                
                Sirius.server.middleware.interfaces.domainserver.CatalogueService ls = (Sirius.server.middleware.interfaces.domainserver.CatalogueService)activeLocalServers.get(lsName);
                ...
                {
                    if (logger!=null) logger.debug("System :"+ls+ " not available");
                    
                    MetaNode error = new MetaNode(0,lsName,lsName +" not available!",lsName+" not available!",true,Policy.createParanoidPolicy(),-1,null,false,-1);
                    
                    error.validate(false);n = new Node[1];n[0] = error;
    ...
    public Node[] getRoots(User user, String localServerName) throws RemoteException
    {
        ...
        return ((Sirius.server.middleware.interfaces.domainserver.CatalogueService)activeLocalServers.get(localServerName)).getRoots(user).getLocalNodes();
    }
    ...
    public Node[] getRoots(User user) throws RemoteException
    {
        ...
        try
        {
            ...
            NodeReferenceList children = (NodeReferenceList)((Sirius.server.middleware.interfaces.domainserver.CatalogueService)iter.next()).getRoots(user);
        ...
        {
            if (logger!=null) logger.error("<CS> getTopNodes(user):",e);
    ...
    public Node addNode(Node node, Link parent, User user) throws RemoteException
    {
        ...
        return ( (Sirius.server.middleware.interfaces.domainserver.CatalogueService)activeLocalServers.get(node.getDomain()) ).addNode(node,parent,user);
    }
    
    public boolean deleteNode(Node node, User user) throws RemoteException
    {
        logger.debug("delete Node called node:"+node+ " user::"+user);
        return ( (Sirius.server.middleware.interfaces.domainserver.CatalogueService)activeLocalServers.get(node.getDomain()) ).deleteNode(node,user);
    }
    ...
    public boolean addLink(Node from,Node to, User user) throws RemoteException
    {
        ...
        return ( (Sirius.server.middleware.interfaces.domainserver.CatalogueService)activeLocalServers.get(from.getDomain()) ).addLink(from,to,user);
    }
    ...
    public boolean deleteLink(Node from,Node to,User user) throws RemoteException
    {
        ...
        return ( (Sirius.server.middleware.interfaces.domainserver.CatalogueService)activeLocalServers.get(from.getDomain()) ).deleteLink(from,to,user);
    }
