...
/**
 * NullMetaObject.NULL is representing a metaobject that does not
 * exist in the database. The LWMO-cache returns null instead of trying
 * to fetch the object if the cachehint equals the NullMetaObject.NULL.
 * 
 * If the LWMO cache would store "null", then there would be no way
 * to know if the cachehit has been cleaned up (high memory usage) and just
 * needs to be refetched, or if the cachehit actually stores a null value.
 *
 * @author   thorsten
