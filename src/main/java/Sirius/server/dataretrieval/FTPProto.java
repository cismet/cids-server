 ...
 * DataObject zur\u00FCck.
 *
 * <p>Bezeichnung dieses Protokols ist "ftp".</p>
 *
 * @author   awindholz
...
public class FTPProto implements MetaObjectProto {
    ...
    public DataObject getDataObject(final MetaObject metaDataObject) throws DataRetrievalException {
        ...
        final String login = (String)protoDelegator.getSingleValue(param_mo, "login");//NOI18N
        final String password = (String)protoDelegator.getSingleValue(param_mo, "password");//NOI18N

        final String fileName = metaDataObject.getName();

        if (fileName == null) {
            final String message = "Name of dataobjekt was not found."//NOI18N
...
/*            String[] userData = getUserData(url);
            ...
            data = getDataObject(remoteHost, controllPort, login, password, path);
        } catch (IOException e) {
     ...
     * @param   controlPort  DOCUMENT ME!
     * @param   kennung      DOCUMENT ME!
