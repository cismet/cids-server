...
//                logger.debug("searchOptions constructed :: "+new Vector(searchOptions.values()));
    ...
    private final void setClassPermissions(String domain)
    {
    ...
    private final void setUserGroupPermissions(String domain)
    {
        ...
        try
        {
            ...
            ResultSet rs = getQueryClassAssoc.executeQuery("SELECT d.name,query_id,ug_id from cs_query_ug_assoc as uga ,cs_domain as d , cs_ug as ug where uga.ug_id=ug.id and d.id=ug.domain");
    ...
    private final void linkIt(String domain)
    {
        ...
        try
        {
            ...
            ResultSet rs = getLinks.executeQuery("SELECT id_from,id_to,d.name as domain_to,qfrom.name as fromName,qto.name as toName from cs_query_link as l,cs_query as qfrom,cs_query as qto,cs_domain as d where l.id_from = qfrom.id and l.id_to=qto.id and d.id=l.domain_to");
    ...
    private final void setSearchParameters(String domain)
    {
        ...
        try
        {
            ...
            ResultSet rs = getSearchParameter.executeQuery("SELECT qp.id as p_id,q.id as q_id,qp.param_key as p_key,qp.is_query_result as is_result,qp.query_position as pos,q.name as query_name, qp.descr as query_description from cs_query_parameter as qp,cs_query as q where q.id=qp.query_id");
    ...
    public int addQuery(String name,String description,String statement,int resultType,char isUpdate, char isBatch,char isRoot,char isUnion)  throws Exception
    {
        ...
        String stmnt = "insert into cs_query values("+maxId+",'"+name+"','"+description+"','"+statement+"',"+resultType+",'"+isUpdate+"','"+isBatch+"','"+isRoot+"','"+isUnion+"', false,true )";
    ...
    public boolean addQueryParameter(int queryId,int typeId,String paramKey,String description,char isQueryResult,int queryPosition) throws Exception
    {
        Statement s = con.createStatement();
        int maxId= 0;
        ...
        String stmnt = "insert into cs_query_parameter values("+maxId+","+queryId+",'"+paramKey+"','"+description+"','"+isQueryResult+"',"+typeId+","+queryPosition+")";
